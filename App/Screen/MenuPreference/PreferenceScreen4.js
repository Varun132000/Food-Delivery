...
import { MilkShakesData } from '../Data';
import { useDispatch, useSelector } from 'react-redux';
import { colors } from '../../Components/Style';
import { addToCart, decrementQuantity, incrementQuantity, removeFromCart } from '../../Redux/CartReducer';
const PreferenceScreen4 = ({ navigation, route }) => {
    const index = route.params.index;
    const [addItems, setAddItems] = useState(0);
    const [selected, setSelected] = useState(false);

    const { meal, details, price,image } = MilkShakesData[index];
    ...
    const renderPrefenceData = () => {
        return (
            <View>
                {MilkShakesData[index].preferenceData.map((item) => (
                    <View key={item.id}>
                        <View style={styles.preferData}>
                            <Text style={styles.preferDataText}>{MilkShakesData[index].preferenceTitle[MilkShakesData[index].preferenceData.indexOf(item)]}</Text>
                            {MilkShakesData[index].required[MilkShakesData[index].preferenceData.indexOf(item)] && (
                                <View style={styles.menuData}>
                                    <Text style={styles.menuDataText}>{MilkShakesData[index].minimum_quatity[MilkShakesData[index].preferenceData.indexOf(item)]} REQUIRED</Text>
                                </View>
                            )}
                        </View>
...
export default PreferenceScreen4;
