...
import { ChickenDetails, menuDetailedData, menuItems } from '../../Screen/Data'
import { useDispatch, useSelector } from 'react-redux';
import { colors } from '../../Components/Style';
import { addToCart,  decrementQuantity, incrementQuantity, removeFromCart} from '../../Redux/CartReducer';
const PreferenceScreen2 = ({ navigation, route }) => {
    const index = route.params.index;
    const [addItems, setAddItems] = useState(0);
    const [selected, setSelected] = useState(false);

    const { meal, details, price,image } = ChickenDetails[index];
    ...
    const renderPrefenceData = () => {
        return (
            <View>
                {ChickenDetails[index].preferenceData.map((item) => (
                    <View key={item.id}>
                        <View style={styles.preferData}>
                            <Text style={styles.preferDataText}>{ChickenDetails[index].preferenceTitle[ChickenDetails[index].preferenceData.indexOf(item)]}</Text>
                            {ChickenDetails[index].required[ChickenDetails[index].preferenceData.indexOf(item)] && (
                                <View style={styles.menuData}>
                                    <Text style={styles.menuDataText}>{ChickenDetails[index].minimum_quatity[ChickenDetails[index].preferenceData.indexOf(item)]} REQUIRED</Text>
                                </View>
                            )}
                        </View>
